...
[qam16, qam_binary_map] = generate_qam_map(qam_size,q); %this function works for squared QAM 16,64
...
FE_seq = zeros(length(Eb_No_db),1);
...
for i = 1:length(Eb_No_db)
     ...
     while(FE(i) < targetFE && genFrame(i)<max_gen)
        ...
        % this is uncoded error calculation
        for e = 1 : K          
        ...
        AvgIt = iters_cnr(i) / genFrame(i);

        frame_error = any(seqgf(1:K) ~= code_seq(1:K));
     ...
     BER= BE_Coded(i)/(genFrame(i)*K*p);
     BER_uncoded = BE_unCoded(i)/(genFrame(i)*K*p);

    fprintf('Eb/No=%.1f dB: FER=%.3e, Coded BER=%.3e, Uncoded BER=%.3e, AvgIt=%.2f\n', ...
    Eb_No_db(i), FER, BER, BER_uncoded, AvgIt);
end
...
ems_ber=[0.0330501,0.00532898,0.000337068,4.8366e-06];
 ...
 xlabel('Eb/No (dB)'); 
 ylabel('BER'); 
 %title('BER curve with noise perturbation');
 hold off;

legend(sprintf('NG-CNV-SF'), 'EMS', 'Uncoded', 'Location', 'northeast');
